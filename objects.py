# Objects used in gui and pdf files

class Board:
    def __init__(self, label='Board', real_valueX=2600, real_valueY=2070, real_valueZ=18, valueX=0, valueY=0, notes='No notes', generate = False, direction = False):
        self.label = label
        self.real_valueX = real_valueX
        self.real_valueY = real_valueY
        self.real_valueZ = real_valueZ
        self.valueX = valueX
        self.valueY = valueY
        self.notes = notes
        self.generate = generate
        self.direction = direction
    
    def __str__(self):
         return (f'BoardClass(Label= {self.label}, valueX= {self.valueX}, valueY= {self.valueY}, '
                 f'valueZ= {self.valueZ},notes= {self.notes},generate= {self.generate},direction= {self.direction})')

class Saw:
    def __init__(self, label='Saw', valueX=0, real_valueX = 3, notes='No notes', generate = False):
        self.label = label
        self.valueX = valueX
        self.real_valueX = real_valueX
        self.notes = notes
        self.generate = generate
    
    def __str__(self):
        return (f'SawClass(Label= {self.label}, valueX= {self.valueX}, real_valueX= {self.real_valueX}, '
                f'notes= {self.notes},generate= {self.generate})')

class Cut:
    def __init__(self, label='Cut', valueX=0, valueY=0, generate = False, posX=0, posY=0,
                  mainboard=1, real_valueX = 0, real_valueY = 0, notes = 'No notes', count = 1, direction = False):
        self.label = label
        self.real_valueX = real_valueX
        self.real_valueY = real_valueY 
        self.generate = generate
        self.posX = posX
        self.posY = posY
        self.mainboard = mainboard
        self.valueX = valueX
        self.valueY = valueY
        self.notes = notes
        self.count = count
        self.direction = direction

    def __str__(self):
        return (f'CutClass(Label= {self.label}, real_valueX= {self.real_valueX}, real_valueY= {self.real_valueY}, '
                f'generate= {self.generate},posX= {self.posX}, posY= {self.posY}, mainboard= {self.mainboard}, '
                f'valueX = {self.valueX}, valueY= {self.valueY}, notes= {self.notes}, count= {self.count}, '
                f'direction= {self.direction})')

class Incision:
        def __init__(self, posX=0, posY=0, posX2=0, posY2=0, mainboard=0):
            self.posX = posX
            self.posY = posY
            self.posX2 = posX2
            self.posY2 = posY2
            self.mainboard = mainboard